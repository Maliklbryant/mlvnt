...
/* ── AUTH LOGIN ──────────────────────────────────────────────────────────── */
function AuthLogin({ onLoginSuccess, onForgot, onSignup, onConsult, onPackages }) {
...
function AuthForgot({ onBack }) {
  ...
  const verifyToken=()=>{
    const tok=(token.trim()||resetToken);const em=RESET_STORE.consume(tok);
    ...
    <div className="auth-shell">
      ...
      <div className="auth-card page-fade">
        ...
        {step===1&&(<>
          ...
          <div className="field mt-16"><label className="field-label">Reset Token</label><input className="fi" placeholder="Paste token from your email link" value={token||resetToken} onChange={e=>{setToken(e.target.value);setErr("");}} /><p className="field-note">Demo: token is pre-filled. In production this arrives via email link.</p></div>
...
export default function App() {
  ...
  return (
    <>
      ...
      {screen === "consult"        && <ConsultationFlow onBack={()=>setScreen("login")} onComplete={()=>setScreen("recommendation")} />}
      {screen === "recommendation" && <ConsultationRecommendation onBack={()=>setScreen("consult")} onProceed={()=>setScreen("onboarding")} />}
      {screen === "packages"       && <PackagePricing onBack={()=>setScreen("login")} onConsult={()=>setScreen("consult")} />}
