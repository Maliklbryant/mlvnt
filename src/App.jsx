...
.page-body{flex:1;padding:32px 36px;overflow-y:auto;}
...
/* ── MESSAGES ── */
.msg-layout{display:grid;grid-template-columns:260px 1fr;flex:1;overflow:hidden;min-height:0;}
...
.msg-chat{display:flex;flex-direction:column;overflow:hidden;min-height:0;}
...
.msg-input{flex:1;}
@media(max-width:800px){.msg-layout{grid-template-columns:1fr;}.msg-list{display:none;}.msg-chat{height:100%;flex:1;}}
              ...
              <div className="slots-wrap">
                ...
                {selTime && !booked && slotStatuses[selTime]?.status === "available" && (
                  <div className="confirm-card">
                    ...
                    {bookErr && (
                      <p style={{fontSize:"0.72rem",color:"rgba(220,100,100,0.9)",marginTop:8,textAlign:"center"}}>{bookErr}</p>
                    )}
                  </div>
                )}
...
function Messages({ session, onRead, onBack }) {
  ...
  useEffect(() => {
    ...
    getCoachId().then(id => {
      ...
      setMsgErr("Could not connect to messaging. Try again.");
      setLoad(false);
    });
...
/* ── ADMIN SCHEDULE ──────────────────────────────────────────────────────── */
function AdminSchedule({ session }) {
  ...
  // Load saved availability from Supabase on mount
  useEffect(() => {
    if (!session?.id) return;
    getCoachAvailability(session.id).then(data => {
      if (data?.weekly_schedule) setAvail(data.weekly_schedule);
      if (data?.blocked_windows) setBlocks(data.blocked_windows);
    }).catch(() => {});
  }, [session?.id]);

  const saveAvailability = async () => {
    setSaveErr("");
    const result = await saveCoachAvailability(session?.id, avail, blocks);
...
function AdminPackages({ dbClients }) {
  ...
  const applyAdj = async (clientId, sign) => {
    ...
    if (result.ok) {
      ...
      // Notify client their package was updated
      createNotification({
        recipientId: clientId,
        type: "package_updated",
        title: "Session balance updated",
      ...
      setClientInv(p => ({
        ...p,
        [clientId]: { ...p[clientId], balance: (clientInv[clientId]?.balance ?? 0) }
      }));
    }
...
function ConsultationFlow({ onBack, onComplete }) {
  ...
  const submit = async () => {
    ...
    // Build ISO date string (YYYY-MM-DD) for Supabase — not the display string
    let isoDate = null;
    if (selDate) {
      const d = new Date(yr, now.getMonth(), selDate);
...
function AdminConsultations({ setView: setAdminView, session, onReviewed }) {
  ...
  useEffect(() => {
    loadLeads();
    // Realtime: reload when a new consultation request is inserted
    const sub = supabase
      .channel("consultation_requests:admin")
      .on("postgres_changes", {
            ...
            <div style={{display:"flex",justifyContent:"center"}}>
              <div style={{position:"relative"}}>
                <div className="site-about-av">
                  {/* Initials shown as fallback when image hasn't loaded */}
                  <span style={{
                    position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",
                    fontFamily:"var(--fh)",fontSize:"2.6rem",fontWeight:800,letterSpacing:"-0.04em",
                    color:"rgba(255,255,255,0.18)",userSelect:"none",zIndex:0,
                  }}>MB</span>
                  <img
                    src="/Malik.jpg"
